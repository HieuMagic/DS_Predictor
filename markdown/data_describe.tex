...
\title{\textbf{Mô tả Dữ Liệu}}
\author{
    Nguyễn Quang Duy \and Lưu Trọng Hiếu \and Văn Đình Hiếu
}
\date{\today}
...
\begin{itemize}
    \item \textbf{Số lượng mẫu}: 14,585 xe
    \item \textbf{Số lượng đặc trưng}: 71 features (70 features + 1 target)
    ...
    \begin{itemize}
        ...
        \item Sau xử lý outliers và missing values: 14,585 mẫu
    \end{itemize}
\end{itemize}
...
\textbf{Chotot:}
\begin{itemize}
    \item \texttt{Còn hạn đăng kiểm} (58.3\% missing)
    \item \texttt{Số đời chủ} (57.0\% missing)
    \item \texttt{Có phụ kiện đi kèm} (67.6\% missing)
...
\begin{itemize}
    ...
    \item Số features: 21 cột
    \item Tình trạng: Nhiều missing values (17\%-100\%), dữ liệu chưa chuẩn hóa
\end{itemize}
...
Số chỗ & 11.1\% & GIỮ & Impute theo body\_type \\
location & 3.0\% & GIỮ & Fill "Khac" \\
Trọng lượng, Trọng tải & 4.3\% & BỎ & Ít liên quan đến giá \\
Chính sách bảo hành & - & BỎ & Không liên quan đến giá \\
...
\begin{itemize}
    ...
    \item Tình trạng: Dữ liệu sạch hơn, hầu như không có missing values (chỉ 0.15\% ở location)
\end{itemize}
...
\begin{itemize}
    \item Chỉ có cột \texttt{location} có 15 giá trị thiếu (0.15\%) - tỷ lệ cực thấp
...
\begin{itemize}
    ...
    \item \textbf{Output}: \texttt{normalize\_interim.csv} (14,928 rows)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Kết quả}: Giảm từ 14,928 $\rightarrow$ 14,585 rows (loại bỏ $\sim$343 duplicates)
\end{itemize}
...
\begin{itemize}
    \item \textbf{Trước}: 14,928 rows
    \item \textbf{Sau}: 14,585 rows
    \item \textbf{Loại bỏ}: 343 outliers (2.3\%)
\end{itemize}
...
seats & 843 & 5.78\% & Lookup từ body\_type \\
body\_type & 736 & 5.05\% & Mode theo model \\
km & 229 & 1.57\% & Xe mới$\rightarrow$0, xe cũ$\rightarrow$median \\
...
\subsubsection{Kết quả}
\begin{itemize}
    \item \textbf{Sau imputation}: 0\% missing values
    \item \textbf{Dataset}: 14,585 rows $\times$ 15 cols (clean)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Phân bố}: $\sim$18.7\% xe sang trong dataset (2,723 xe)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Phân bố}:
    \begin{itemize}
        \item Low (<10,000 km/năm): 53.8\% (7,067 xe)
...
\begin{itemize}
    ...
    \item Xóa cột \texttt{seller\_id} (chỉ dùng để phát hiện duplicates, không có giá trị dự đoán)
\end{itemize}
...
\textbf{b) \texttt{usage} $\rightarrow$ \texttt{usage\_*} (3 features)}
\begin{itemize}
...
\begin{itemize}
    \item \texttt{brand}, \texttt{model}, \texttt{transmission}, \texttt{fuel\_type}, \texttt{body\_type}, \texttt{origin}, \texttt{condition}, \texttt{city}, \texttt{source}, \texttt{usage}
...
\begin{itemize}
    \item \textbf{Số mẫu}: 14,585 xe (sau loại bỏ duplicates và outliers)
    \item \textbf{Số features}: 71 (70 features + 1 target)
    \item \textbf{Kiểu dữ liệu}:
    \begin{itemize}
        \item Integer: 67 features (binary và one-hot encoded)
        \item Float: 4 features (engine, km\_per\_year, model\_encoded, price\_million)
    \end{itemize}
...
\midrule
price\_million & int64 & Giá xe (triệu VNĐ) & 50 - $\sim$15,000 & $\sim$700-800 \\
\bottomrule
\end{tabular}
\end{table}
...
is\_luxury & int64 & Xe sang hay không & 0 (thường) / 1 (sang) & $\sim$10\% xe sang \\
...
\midrule
\endhead
price\_million & 50 & 380 & 580 & 950 & $\sim$15,000 & $\sim$750 & $\sim$600 \\
